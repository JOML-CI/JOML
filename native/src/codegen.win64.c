...
enum {
  ...
  GLOB_matrix_scale,
  GLOB_matrix_translate,
  GLOB__MAX
};
#line 11 "codegen.dasc"
//|.actionlist actionlist
static const unsigned char actionlist[1147] = {
  ...
  41,136,233,65,15,41,144,233,65,15,41,152,233,255,248,21,255,15,40,1,72,131,
  193,16,15,40,200,15,198,201,235,65,15,40,16,15,89,209,65,15,41,17,15,40,200,
  15,198,201,235,65,15,40,144,233,15,89,209,65,15,41,145,233,15,40,200,15,198,
  201,235,65,15,40,144,233,15,89,209,65,15,41,145,233,65,15,40,136,233,65,15,
  41,137,233,255,248,22,255,15,40,1,72,131,193,16,15,40,200,15,198,201,235,
  65,15,40,16,15,89,209,15,40,200,15,198,201,235,65,15,40,152,233,15,89,217,
  15,88,211,15,40,200,15,198,201,235,65,15,40,152,233,15,89,217,15,88,211,65,
  15,40,136,233,15,88,209,65,15,40,0,65,15,41,1,65,15,40,136,233,65,15,41,137,
  233,65,15,40,128,233,65,15,41,129,233,65,15,41,145,233,255,72,199,194,247,
  255,252,233,244,11,255,252,255,226,255,252,233,244,12,255,252,233,244,10,
  255,252,233,244,13,255,252,233,244,16,255,252,233,244,17,255,252,233,244,
  18,255,252,233,244,20,255,252,233,244,15,255,252,233,244,14,255,252,233,244,
  21,255,252,233,244,22,255,249,255,195,255
};
...
#line 422 "codegen.dasc"
  ...
  //| add rcx, 8
  dasm_put(Dst, 3);
#line 425 "codegen.dasc"
  // obtain dest matrix address
  //| mov r9, [rcx]
  //| add rcx, 8
  dasm_put(Dst, 11);
#line 428 "codegen.dasc"
  ...
  //| movaps xmm1, [r8+4*12]
  //| movaps [r9+4*12], xmm1
  dasm_put(Dst, 896, _MM_SHUFFLE(0, 0, 0, 0), _MM_SHUFFLE(1, 1, 1, 1), 4*4, 4*4, _MM_SHUFFLE(2, 2, 2, 2), 4*8, 4*8, 4*12, 4*12);
...
#line 464 "codegen.dasc"
  ...
  //| movaps [r9+4*8], xmm0
  //| movaps [r9+4*12], xmm2
  dasm_put(Dst, 975, _MM_SHUFFLE(0, 0, 0, 0), _MM_SHUFFLE(1, 1, 1, 1), 4*4, _MM_SHUFFLE(2, 2, 2, 2), 4*8, 4*12, 4*4, 4*4, 4*8, 4*8, 4*12);
...
batch_func_t codegen(const unsigned char* opcodes, int opcodesLength, size_t* codeSize) {
  ...
  for (int i = 0; i < opcodesLength; i++) {
    //| mov rdx, =>next_pc
    dasm_put(Dst, 1074, next_pc);
#line 510 "codegen.dasc"
    switch (opcodes[i]) {
    case OPCODE_MATRIX_MUL_MATRIX:
      //| jmp ->mul_matrix_matrix
      dasm_put(Dst, 1079);
#line 513 "codegen.dasc"
      if (!op_generated[0]) {
        mul_matrix_matrix(&state);
        //| jmp rdx
        dasm_put(Dst, 1084);
#line 516 "codegen.dasc"
        op_generated[0] = 1;
      }
      break;
    case OPCODE_MATRIX_MUL_VECTOR:
      //| jmp ->mul_matrix_vector
      dasm_put(Dst, 1088);
#line 521 "codegen.dasc"
      if (!op_generated[1]) {
        mul_matrix_vector(&state);
        //| jmp rdx
        dasm_put(Dst, 1084);
#line 524 "codegen.dasc"
        op_generated[1] = 1;
      }
      break;
    case OPCODE_MATRIX_TRANSPOSE:
      ...
      dasm_put(Dst, 1093);
#line 529 "codegen.dasc"
      if (!op_generated[2]) {
        matrix_transpose(&state);
        //| jmp rdx
        dasm_put(Dst, 1084);
#line 532 "codegen.dasc"
        op_generated[2] = 1;
      }
      break;
    case OPCODE_MATRIX_INVERT:
      // Not yet implemented!
      break;
    case OPCODE_TRANSLATION_ROTATE_SCALE:
      //| jmp ->translation_rotate_scale
      dasm_put(Dst, 1098);
#line 540 "codegen.dasc"
      if (!op_generated[4]) {
        translation_rotate_scale(&state);
        //| jmp rdx
        dasm_put(Dst, 1084);
#line 543 "codegen.dasc"
        op_generated[4] = 1;
      }
      break;
    case OPCODE_ROTATEZ:
      //| jmp ->rotateZ
      dasm_put(Dst, 1103);
#line 548 "codegen.dasc"
      if (!op_generated[5]) {
        rotateZ(&state);
        //| jmp rdx
        dasm_put(Dst, 1084);
#line 551 "codegen.dasc"
        op_generated[5] = 1;
      }
      break;
    case OPCODE_VECTOR_NEGATE:
      //| jmp ->vector_negate
      dasm_put(Dst, 1108);
#line 556 "codegen.dasc"
      if (!op_generated[6]) {
        vector_negate(&state);
        //| jmp rdx
        dasm_put(Dst, 1084);
#line 559 "codegen.dasc"
        op_generated[6] = 1;
      }
      break;
    case OPCODE_MATRIX_ROTATE_QUATERNION:
      //| jmp ->matrix_rotate_quaternion
      dasm_put(Dst, 1113);
#line 564 "codegen.dasc"
      if (!op_generated[7]) {
        matrix_rotate_quaternion(&state);
        //| jmp rdx
        dasm_put(Dst, 1084);
#line 567 "codegen.dasc"
        op_generated[7] = 1;
      }
      break;
...
#line 575 "codegen.dasc"
    ...
    case OPCODE_ROTATEX:
      //| jmp ->rotateX
      dasm_put(Dst, 1123);
#line 580 "codegen.dasc"
      if (!op_generated[10]) {
        rotateX(&state);
        //| jmp rdx
        dasm_put(Dst, 1084);
#line 583 "codegen.dasc"
        op_generated[10] = 1;
      }
      break;
    case OPCODE_ROTATEY:
      //| jmp ->rotateY
      dasm_put(Dst, 1128);
#line 588 "codegen.dasc"
      if (!op_generated[11]) {
        rotateY(&state);
        //| jmp rdx
        dasm_put(Dst, 1084);
#line 591 "codegen.dasc"
        op_generated[11] = 1;
      }
      break;
    case OPCODE_MATRIX_SCALE:
      //| jmp ->matrix_scale
      dasm_put(Dst, 1133);
#line 596 "codegen.dasc"
      if (!op_generated[12]) {
        matrix_scale(&state);
        //| jmp rdx
        dasm_put(Dst, 1084);
#line 599 "codegen.dasc"
        op_generated[12] = 1;
      }
      break;
    case OPCODE_MATRIX_TRANSLATE:
      ...
      dasm_put(Dst, 1138);
#line 604 "codegen.dasc"
      if (!op_generated[13]) {
        matrix_translate(&state);
        //| jmp rdx
        dasm_put(Dst, 1084);
#line 607 "codegen.dasc"
        op_generated[13] = 1;
      }
      break;
    default:
    ...
    //|=>next_pc:
    dasm_put(Dst, 1143, next_pc);
#line 614 "codegen.dasc"
    next_pc++;
  }
  //| ret
  dasm_put(Dst, 1145);
#line 617 "codegen.dasc"
