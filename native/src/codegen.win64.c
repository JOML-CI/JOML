...
static const unsigned char actionlist[328] = {
  ...
  152,233,15,89,220,15,88,193,15,88,211,15,88,194,255,248,10,255,76,139,1,72,
  129,193,239,255,76,139,9,255,65,15,40,129,233,255,15,40,232,255,15,40,252,
  240,255,15,40,252,248,255,65,15,41,40,65,15,41,176,233,65,15,41,184,233,65,
  15,41,128,233,255,72,129,193,239,252,255,226,255,76,139,1,65,15,40,0,255,
  15,40,200,15,198,201,235,15,40,208,15,198,210,235,15,40,216,15,198,219,235,
  15,40,224,15,198,228,235,255,72,129,193,239,76,139,9,255,65,15,40,1,15,89,
  193,255,65,15,40,137,233,15,89,202,255,65,15,40,145,233,15,89,211,255,65,
  15,40,153,233,15,89,220,255,65,15,41,0,255,252,233,244,11,248,11,255,72,199,
  194,247,255,249,255,195,255
};
...
#line 76 "codegen.dasc"
  ...
  //| mov r8, [rcx]
  //| add rcx, sizeof(char*)
  dasm_put(Dst, 167, sizeof(char*));
#line 79 "codegen.dasc"
...
#line 91 "codegen.dasc"
      ...
      //| movaps [r8+8*4], xmm7
      //| movaps [r8+12*4], xmm0
      dasm_put(Dst, 199, 4*4, 8*4, 12*4);
#line 96 "codegen.dasc"
    }
  }
  // advance args pointer
  //| add rcx, sizeof(char*)
  //| jmp rdx
  dasm_put(Dst, 219, sizeof(char*));
...
batch_func_t codegen(const char* opcodes, int opcodesLength) {
  ...
  int dynamic_labels[max_dynamic_labels];
  int dynamic_label = 0;
  int status;
  void* code;
  int next_pc = 0;
...
#line 170 "codegen.dasc"
  for (int i = 0; i < opcodesLength; i++) {
    ...
    case 0x01: // OPCODE_MATRIX_MUL_MATRIX
      //| mov rdx, =>next_pc
      dasm_put(Dst, 319, next_pc);
#line 174 "codegen.dasc"
      if (!op_generated[0]) {
        mul_matrix_matrix(&state);
        op_generated[0] = 1;
      }
      //|=>next_pc:
      dasm_put(Dst, 324, next_pc);
#line 179 "codegen.dasc"
      dynamic_labels[dynamic_label++] = next_pc;
      next_pc++;
      break;
