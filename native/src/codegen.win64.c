...
static const unsigned char actionlist[325] = {
  76,139,1,72,129,193,239,255,65,15,40,0,15,40,208,65,15,40,136,233,15,198,
  193,136,15,198,209,221,65,15,40,152,233,15,40,252,235,65,15,40,176,233,15,
  198,222,136,15,198,252,238,221,15,40,200,15,198,195,136,15,40,226,15,198,
  213,136,15,198,203,221,15,198,230,221,255,65,15,41,0,65,15,41,144,233,65,
  15,41,136,233,65,15,41,160,233,255,252,255,226,255,15,40,200,15,198,201,235,
  15,40,208,15,198,210,235,15,40,216,15,198,219,235,15,40,224,15,198,228,235,
  65,15,40,0,15,89,193,65,15,40,136,233,15,89,202,65,15,40,144,233,15,89,211,
  65,15,40,152,233,15,89,220,15,88,193,15,88,211,15,88,194,255,248,10,255,76,
  139,9,72,129,193,239,255,65,15,40,129,233,255,15,40,232,255,15,40,252,240,
  255,15,40,252,248,255,65,15,41,40,65,15,41,176,233,65,15,41,184,233,65,15,
  41,128,233,255,76,139,1,72,129,193,239,65,15,40,0,255,15,40,200,15,198,201,
  235,15,40,208,15,198,210,235,15,40,216,15,198,219,235,15,40,224,15,198,228,
  235,255,65,15,40,1,15,89,193,255,65,15,40,137,233,15,89,202,255,65,15,40,
  145,233,15,89,211,255,65,15,40,153,233,15,89,220,255,65,15,41,0,255,72,199,
  194,247,255,252,233,244,10,255,249,255,252,233,244,11,249,255,252,233,244,
  12,249,255,195,255
};
...
static void linearProduct(dasm_State** Dst) {
  ...
  //| addps xmm2, xmm3
  //| addps xmm0, xmm2
  dasm_put(Dst, 98, _MM_SHUFFLE(0, 0, 0, 0), _MM_SHUFFLE(1, 1, 1, 1), _MM_SHUFFLE(2, 2, 2, 2), _MM_SHUFFLE(3, 3, 3, 3), 4*4, 4*8, 4*12);
...
static void mul_matrix_vector(dasm_State** Dst) {
  ...
  //| add rcx, sizeof(char*)
  //| movaps xmm0, [r8]
  dasm_put(Dst, 218, sizeof(char*));
#line 123 "codegen.dasc"
  ...
  //| movaps xmm4, xmm0
  //| shufps xmm4, xmm4, _MM_SHUFFLE(3, 3, 3, 3)
  dasm_put(Dst, 230, _MM_SHUFFLE(0, 0, 0, 0), _MM_SHUFFLE(1, 1, 1, 1), _MM_SHUFFLE(2, 2, 2, 2), _MM_SHUFFLE(3, 3, 3, 3));
#line 133 "codegen.dasc"
...
batch_func_t codegen(const char* opcodes, int opcodesLength) {
  ...
  for (int i = 0; i < opcodesLength; i++) {
    ...
    case 0x01: // OPCODE_MATRIX_MUL_MATRIX
      ...
      //| mov rdx, =>next_pc
      dasm_put(Dst, 299, next_pc);
#line 186 "codegen.dasc"
      if (!op_generated[0]) {
      ...
      dasm_put(Dst, 304);
#line 191 "codegen.dasc"
      // define the label that we just jumped to above
      //|=>next_pc:
      dasm_put(Dst, 309, next_pc);
